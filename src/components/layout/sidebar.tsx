...
function grupoTieneRutaActiva(grupo: NavGroup, pathname: string) {
  return grupo.children.some((c) => pathname === c.href || pathname.startsWith(`${c.href}/`));
}
...
export function Sidebar({ logoUrl }: { logoUrl: string | null }) {
  ...
  const [grupoAbierto, setGrupoAbierto] = useState<string | null>(() => {
    const activo = NAV_ITEMS.find((item) => esGrupo(item) && grupoTieneRutaActiva(item, pathname));
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto px-3 pb-4" aria-label="Navegación principal">
          {NAV_ITEMS.map((item) => {
            if (esGrupo(item)) {
                ...
                <div key={item.label}>
                  <button
                    ...
                    className={cn(
                      ...
                      tieneActivo ? "text-ivory" : "text-text-dim hover:bg-surface-hover hover:text-text"
                    )}
                  >
            ...
            return (
              <Link
                ...
                className={cn(
                  ...
                  isActive ? "bg-white/[0.06] text-ivory" : "text-text-dim hover:bg-surface-hover hover:text-text"
                )}
              >
                <span
                  className={cn(
